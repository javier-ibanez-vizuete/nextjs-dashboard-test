...
export default function EditInvoiceForm({ invoice, customers }: { invoice: InvoiceForm; customers: CustomerField[] }) {
	...
	return (
		<form action={formState} aria-describedby="form-error">
			<div className="rounded-md bg-gray-50 p-4 md:p-6">
				...
				<div className="mb-4">
					...
					<div className="relative">
						...
						<UserCircleIcon className="pointer-events-none absolute left-3 top-1/2 h-[18px] w-[18px] -translate-y-1/2 peer-focus:text-gray-900 text-gray-500" />
					</div>
					<div id="customer-error" aria-live="polite" aria-atomic="true">
						{state.errors?.customerId &&
							state.errors.customerId.map((error) => (
								<p className="mt-2 text-sm text-red-500 italic opacity-80" key={error}>
									{error}
								</p>
							))}
					</div>
				...
				<div id="form-error" aria-live="polite" aria-atomic="true">
					{state.message && <p className="mt-2 text-sm text-red-500 italic opacity-80">{state.message}</p>}
				</div>
			</div>
